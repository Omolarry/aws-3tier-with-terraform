...
resource "aws_appautoscaling_target" "ecs_asg" {
  max_capacity       = 4
  min_capacity       = 1
  resource_id        = "service/${var.my_project_name}-${var.environment}-cluster/${var.my_project_name}-${var.environment}-service"
...
resource "aws_appautoscaling_policy" "ecs_policy" {
  name               = "${var.my_project_name}-${var.environment}-cluster/${var.my_project_name}-${var.environment}-policy"
